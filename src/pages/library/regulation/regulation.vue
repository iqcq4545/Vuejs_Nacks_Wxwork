      ...
      <div class="form1 body">
        <div class="wrap">
          <ul class="libTab bgw fs26">
            <li v-for="(item,i) in tab.list" :key="i" :class="i===tab.current?'on':''"
              v-if="item==='文档附件'?ItemOption.data.FileInfo.Name:item" @click="changeTab(i)">
              {{item}}
            </li>
          </ul>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getType() {
        this.$ReqLibrary.regulationType().then((res) => {
      ...
      fileType(str) {
        var list = ["doc", "xls", "doc"];
        for (var i = 0; i < list; i++) {
          if (str.indexOf(list[i])) {
            return true;
          }
        }
        return false;
      ...
      getList(add = 0) {
        if (this.pageOption.max > this.pageOption.index) {
          ...
          this.$ReqLibrary.getRegulation({ pageIndex: 1, pageSize: this.pageOption.index * this.pageOption.size, ruleSystemTypeId: this.SiderOption.selected[0].uid, name: this.SearchOption.keyword }).then((res) => {
      ...
      showContent(i) {
        this.$ReqLibrary.getRegulationById({ id: this.itemList[i].Id }).then((res) => {
