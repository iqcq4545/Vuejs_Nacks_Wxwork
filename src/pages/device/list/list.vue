<template>
  <div class="page">
      ...
      <div class="container item">
        <div v-for="(item,i) in itemList" :key="i" class="row bgw">
          <div class="top" @click="showDetail(i)">
            <div class="img">
              <img :src="item.Pic||require('@/images/item_img.jpg')" />
            </div>
            <dl class="txt">
              <dt>
                <em v-if="item.DeviceStatus.Id" class="statusbg normal">{{item.DeviceStatus.Name}}</em>
                {{item.Name}}
              </dt>
              <dd>
                <h3>设备编码 :</h3>
    ...
    <Detail v-if="DetailOption.isShow" :DetailOption="DetailOption" @chooseImg="chooseImg" @setScroll="setScroll"></Detail>
  </div>
</template>
    ...
    methods: {
      ...
      chooseImg(e) {
        ...
        for (var i = 0; i < files.length; i++) {
          that.$lrz(files[i]).then(function (result) {
            ...
            that.Confirm({
              text: "确认上传新的设备图片？",
              action: { yes: that.updateImg, no: that.noUpdateImg }
            });
          }).catch(error => {
          }).always(() => { });
        }
      ...
      updateImg() {
        ...
        that.$ReqDevice.uploadImg(that.lrzResult.formData).then(res => {
          that.Toast({ text: "图片上传完成，正在修改设备图片", icon: "loading" });
          that.$ReqDevice.updateImg({
            sn: that.DetailOption.data.SN,
            picName: res.data.maskName
          }).then(res => {
        ...
        if (this.pageOption.max > this.pageOption.index) {
          ...
          this.$ReqDevice
            .getList({
              ...
              name: this.SearchOption.keyword,
              random: Math.random()
            })
            .then(res => {
              Array.prototype.push.apply(this.itemList, res.data.list);
              this.pageOption.max = Math.ceil(
                res.data.recordCount / this.pageOption.size
              );
              this.Toast({}, 0);
              this.loading = false;
            });
      ...
      updateList() {
        this.$ReqDevice
          .getList({
            pageIndex: 1,
            pageSize: this.pageOption.index * this.pageOption.size,
            name: this.SearchOption.keyword
          })
          .then(res => {
      ...
      getCancel() {
        ...
        this.$ReqRepair
          .getCancel({ sn: this.itemList[this.selected].SN })
          .then(res => {
