    ...
    methods: {
      ...
      getList(add = 0) {
        ...
        if (this.pageOption.max > this.pageOption.index) {
          ...
          this.$ReqDispatch.getList({ pageIndex: this.pageOption.index, pageSize: this.pageOption.size, createrId: cookiesValue("ZT_DevicePlatForm_UserId"), workOrderTypeId: this.SiderOption.selected[0].uid, flag: 1, statusId: this.SiderOption.selected[1].uid, title: this.SearchOption.keyword, random: Math.random() }).then((res) => {
      ...
      checkCancel(i) {
        this.selected = i;
        this.Confirm({ text: "确认撤销工单？", action: { yes: this.getCancel } });
      },

      getCancel() {
        var item = this.itemList[this.selected];
        this.Toast({ text: "正在撤销", icon: "loading" }, 20000);
        this.$ReqDispatch.getCancel({ businessSN: item.BusinessSN, workOrderTypeId: item.WorkOrderType.Id }).then((res) => {
      ...
      sparePartListPending(i) {
        this.$ReqRepair.sparePartListPending({ workOrderSN: this.itemList[i].SN }).then((res) => {
          var table = [],
            data = res.data.list;
