    ...
    methods: {
      ...
      getList(add = 0) {
        if (this.pageOption.max > this.pageOption.index) {
          ...
          this.$ReqJob.getTeamList({ pageIndex: this.pageOption.index, pageSize: this.pageOption.size, userId: cookiesValue("ZT_DevicePlatForm_UserId"), workOrderTypeId: this.SiderOption.selected[0].uid, statusId: this.SiderOption.selected[1].uid, title: this.SearchOption.keyword, random: Math.random() }).then((res) => {
